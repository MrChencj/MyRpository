			...
			<tr>
				...
				<span style="font-weight: bold;">共${pageCount }页</span>
						<a href="javascript:void(0)"
							onclick="openUrl('./NoticeServlet?oprationType=bgAllNotice&pageNum=1','首页')">首页</a>
						<c:forEach begin="2" end="${pageCount-1 }" step="1" var="num">
							<c:if test="${pageNum!=num }">
								<a href="javascript:void(0)"
							onclick= "openUrl('./NoticeServlet?oprationType=bgAllNotice&pageNum=${num }','第${num }页')">
							</c:if>
						${num }
					<c:if test="${pageNum!=num }">
								</a>
						...
						<a href="javascript:void(0)" onclick="openUrl('./NoticeServlet?oprationType=bgAllNotice&pageNum=${pageCount}', ' 尾页  '  )  " >尾页</a>
					</div></td>
			</tr>
		</table>
	</div>
